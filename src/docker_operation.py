...
task_name, docker_server_url, docker_image, container_name, container_cpu, container_memory = ('','','','', 1024, '1g')
...
def validate_config(task_conf):
    ...
    global docker_server_url, docker_image, container_name, container_ports, container_volumes,container_cpu, container_memory
    ...
    if not task_conf.has_key('docker.image') or not task_conf.has_key('container.name'):
        print 'Image and container name must be provided'
        exit(1)
    
    docker_image = common_util.get_config_value_by_key(task_conf, 'docker.image')
...
def create():
    ...
    container = docker_operation.create_container(docker_image, container_name=container_name, ports=container_ports, volumes=container_volumes, cpu_shares=container_cpu, mem_limit=container_memory)
...
if __name__ == '__main__':
    ...
    print 'Task conf for [%s]: %s | %s | %s | %s | %s | %s | %s' %(task_name, docker_image, container_name, container_cpu, container_memory, container_ports, container_volumes, docker_server_url)
